...
行動設計における諸々の専門用語を集めた用語集です（全76語）。UXデザイン全般に関する用語については別ページにまとめました（[→UXデザイン用語集](../ux-design-glossary/)）
...
note:優れたUXを実現するためにユーザーに特定の行動をとってもらいたい場合がある。そのような場合に、行動設計の知見を用いて行動を誘発する

note:行動設計によって介入対象者の行動を誘発できたとして介入対象者に不快感を与える場合がある。そのような場合に、UXデザインの知見を用いて不快感発生を回避する
...
行動分析学の用語で、「誰が見ても同じ行動だと判断できる程度に具体的な行動だけを分析対象として扱う」とする考え方

[具体性テスト]:#具体性テスト "行動分析学の用語で、「誰が見ても同じ行動だと判断できる程度に具体的な行動だけを分析対象として扱う」とする考え方"

note:具体性テストNGの行動は客観的な測定ができないので、行動分析の対象として扱えない

note:（関連語）[死人テスト]、[具体性テスト]
...
<1>[ナッジ]の言い換え語 <2>[行動設計]に同じ

[行動デザイン]:#行動デザイン "<1>ナッジの言い換え語 <2>行動設計に同じ"
...
作業すること自体が刺激となって作業に対する内発的なモチベーションが生じる現象

[作業興奮]:#作業興奮 "作業すること自体が刺激となって作業に対する内発的なモチベーションが生じる現象"

note:つまり、やる気の出ない作業であっても、まず作業をすることで後からやる気が出てくるということ

note:作業が楽しくなるわけではないことに注意。あくまでも「一般的な傾向として作業開始直後から開始後10分にかけて作業能率が上昇し続ける」という実験的事実から、"何らかの内発的なモチベーション"が生じているとしたもの

note:（関連語）[モチベーション]、[フロー状態]
...
[シカケ]:#仕掛け "（→仕掛け）大阪大学松村教授の提唱した概念。問題解決につながる行動を誘発するシンプルな工夫で、かつ仕掛けの3要件を満たすもの"

note:たとえば「バスケットボールのゴールポストの形をしたゴミ箱」などが仕掛けに該当する
...
行動分析学の用語で、「否定形・受け身形・状態で表現されることは行動として扱わない」とする考え方。「死人でもできること」という意味で死人テストと呼ばれる

[死人テスト]:#死人テスト "行動分析学の用語で、「否定形・受け身形・状態で表現されることは行動として扱わない」とする考え方"

note:死人テストNGの事象は意図的な行動か偶然の結果かを観察者が判断できないので、行動分析の対象として扱えない
...
行動設計は「人の非合理的行動が原因で生じる問題」を主な対象としています。たとえば「暴飲暴食をやめられず、病気になる」「安全確認をさぼったため、事故を起こす」などが人の非合理的行動が原因で生じる問題に該当します。
...
行動設計の手法は悪用可能です。たとえば悪徳企業が消費者にムダ遣いさせるために行動設計の手法を応用することも可能です。また、悪意はなくとも行動設計がたんなる価値観の押し付けになってしまう可能性もあり得ます。行動設計を行う技術者には、高い倫理性が求められます。

### 使いやすさと行動設計

道具を使う行動においては、道具の使いづらさは行動阻害要因になり得ます。行動の変容を促すためには使いやすさに配慮した設計が必要です。

ただし、意図的に使いづらく設計する場合もあります。危険行為を阻止したい時やビデオゲームで達成感を演出したい時などは、阻止したい行動に関する箇所のみ、意図的に使いづらく設計されます。
